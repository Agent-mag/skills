...
**Flutter Handling Http And Json** is an installable Agent Mag skill bundle. It gives your coding agent a focused prompt, optional tool definitions, and lightweight configuration so it can perform this job consistently inside your workflow.
...
```bash
npx agentmag add skill flutter-handling-http-and-json
```
...
- [Examples](#examples)
- **Enforce HTTPS:** iOS and Android disable cleartext (HTTP) connections by default. Always use HTTPS endpoints. If HTTP is strictly required
...
- **License:** MIT
- **Slug:** `flutter-handling-http-and-json`
